...
> 原为 `PROJECT_HANDOFF.md` §8，2026-07-21 迁出为独立文件。
...
### 2026-07-21 · Handoff 文档按社区规范重构

**变更内容**

- 新建 `.claude/HANDOFF.md` — 按照社区规范格式的精简 handoff（≤1500 tokens）
- 新建 `docs/changelog.md` — 从 `PROJECT_HANDOFF.md` §8 迁出全部变更记录
- `PROJECT_HANDOFF.md` 改为精简索引/重定向页
- `CLAUDE.md` 中所有对 `PROJECT_HANDOFF.md` 的引用更新为 `.claude/HANDOFF.md`
...
### 2026-07-19 · 合并 feature/card-redesign

合并 feature/card-redesign → master（首页/工具大全页卡片 tagline/tags/difficulty 展示 + 新样式）。
...
- 其余 24 个 tool-registry 占位继续作为 P7+ backlog

**影响**

- 4 个 lead tool 全部走「spec → plan → TDD 实施 → code-review → commit」标准流程
- 4 份 plan 全部落地，共 **15 task**（DNS 6 / CPU 7 / SHA-256 6 / B+ 6）
...
- **代码风格层（3 项）**全部落地：
  - `var` → `const`/`let` 全迁移（Task 7，8 文件）
  - `pages/sort-viz/sort-viz.js` 抽出 sort-step 生成器到 `utils/sort-algorithms.js`（Task 8）
  - `tests/pages/sort-viz.test.js` 改为 import 真实实现，删除 92 行 duplicating（Task 9）

**理由**

- spec 经 brainstorming → writing-plans → 用户批准全流程
...
- 项目进入「约定文档化」状态
- `tests/` 下 ~222 处 `var` 仍待清理（r.8 候选；不在本次 plan 范围）

参见：`docs/plans/2026-07-11-normalization.md` · `docs/superpowers/specs/2026-07-11-normalization-design.md`

### 2026-07-11 · var 全清零 plan 实施完成（8 Task · 7 commit）

**变更内容**

- 范围：仅 `utils/` + `pages/`（`tests/` 不在范围内）
- 7 文件 / **283 处 var → const/let**，按方案 B 每文件 1 commit：
...
- 全程 `npm test` gate 全绿
- subagent 主动识别 brief 错误，避免运行时 `TypeError: Assignment to constant variable`

**影响**

- `utils/` + `pages/` 内 `grep -rnE '\bvar\b'` 已无输出（仅历史 docs 与 tests/ 仍含）

参见：`docs/plans/2026-07-11-var-cleanup.md` · `docs/superpowers/specs/2026-07-11-var-cleanup-design.md`
...
- 剩余 47 High + 53 Medium 项：详见报告 §10
...
- 根因：`pages/deadlock/deadlock.wxml` 第 177 行，内层 `wx:for` 循环中，`{{item}}` 放错了位置

**变更**
- `pages/deadlock/deadlock.wxml` L177：`{{item}}` 从属性区移到 `<view>` 标签内容区
