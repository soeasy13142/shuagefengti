...
>
> 实现状态速览:
> - ✅ C 类语言 Token 化（12 个关键字、字符串、数字、运算符、分隔符、注释）
> - ✅ 逐字符动画
> - ✅ Token 流展示（类型/词素/行号/列号）
> - ✅ 符号表（标识符出现位置汇总）
> - ✅ 最长匹配原则
> - ✅ 错误处理（非法字符、未闭合注释）
> - ✅ 步进模式 / 全部分析 / 重置
> - ✅ 5 个预设示例
> - ❌ **未实现**: 自定义正则规则编辑与匹配测试 UI
> - ✅ Claude Design 色调
...
- ✅ 最长匹配原则演示（如 `>=` 优于 `>` 后匹配 `=`）
- ❌ 正则规则编辑与匹配测试（用户可自定义简单 Token 正则）— **未实现**
...
6. ✅ **预设示例**：提供 5 个预设代码片段（变量声明、函数调用、嵌套注释、多行字符串等）。
