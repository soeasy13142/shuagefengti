...
> 状态：待用户审核
> 关联：`utils/tool-registry.js`（`sync-viz` 占位）、`app.json`
...
| 风险 | 缓解 |
|---|---|
| 信号量等待队列的动画表现抽象 | 等待队列以列表展示在信号量条下方，标注 "P2 阻塞中" |
| 步进模式与自动播放模式的状态同步 | 统一使用 step 队列，两种模式只是触发方式不同 |
| 缓冲区满/空时的阻塞表现不够直观 | 阻塞时生产者/消费者的图标变成灰色 + "⏳" 动画，直到被唤醒 |
