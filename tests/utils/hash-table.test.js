...
describe('htInsert', () => {
  test('不冲突时直接落入正确桶', () => {
    let table = createHashTable(7);
    let result = htInsert(table, 'apple', 3);
    let bucketIndex = hash('apple', 7);
  ...
  test('冲突时链表变长', () => {
    ...
    if (key1 && key2) {
      let result1 = htInsert(table, key1, 'v1');
      let result2 = htInsert(result1.table, key2, 'v2');
      let bucketIndex = hash(key1, 7);
  ...
  test('步骤包含 hash-calc 类型', () => {
    let table = createHashTable(7);
    let result = htInsert(table, 'apple', 3);
    let hashCalcStep = result.steps.find(function(s) { return s.type === 'hash-calc'; });
  ...
  test('步骤包含 insert 类型', () => {
    let table = createHashTable(7);
    let result = htInsert(table, 'apple', 3);
    let insertStep = result.steps.find(function(s) { return s.type === 'insert'; });
    expect(insertStep).toBeDefined();
  });

  test('冲突时步骤包含 collision 类型', () => {
    let table = createHashTable(2);
    let r1 = htInsert(table, 'ab', 1);
    let r2 = htInsert(r1.table, 'ba', 2);
    let collisionStep = r2.steps.find(function(s) { return s.type === 'collision'; });
...
describe('htSearch', () => {
  test('查找存在的 key 返回 found', () => {
    let table = createHashTable(7);
    let inserted = htInsert(table, 'apple', 3);
    let result = htSearch(inserted.table, 'apple');
  ...
  test('查找不存在的 key 返回 not-found', () => {
    let table = createHashTable(7);
    let result = htSearch(table, 'apple');
...
describe('cloneTable', () => {
  test('深拷贝产生独立副本', () => {
    let table = createHashTable(7);
    let inserted = htInsert(table, 'apple', 3);
    let cloned = cloneTable(inserted.table);
