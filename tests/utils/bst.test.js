...
function buildTree(values) {
  ...
  for (var i = 0; i < values.length; i++) {
    var result = insertNode(root, values[i]);
    root = result.root;
  }
  return root;
}
...
describe('insertNode', () => {
  ...
  test('插入到已有节点的左侧生成 compare 步骤', () => {
    ...
    var result = insertNode(root, 30);
    var compareSteps = result.steps.filter(function(s) { return s.type === 'compare'; });
...
describe('inOrderValues', () => {
  test('中序遍历结果有序', () => {
    var root = buildTree([50, 30, 70, 20, 40, 60, 80]);
...
describe('searchNode', () => {
  test('查找存在的节点返回 found 步骤', () => {
    var root = buildTree([50, 30, 70, 20, 40]);
    var steps = searchNode(root, 40);
    var foundStep = steps.find(function(s) { return s.type === 'found'; });
  ...
  test('查找不存在的节点返回 not-found 步骤', () => {
    var root = buildTree([50, 30, 70]);
    var steps = searchNode(root, 25);
    var notFoundStep = steps.find(function(s) { return s.type === 'not-found'; });
...
describe('deleteNode', () => {
  test('删除叶子节点', () => {
    var root = buildTree([50, 30, 70, 20]);
    var result = deleteNode(root, 20);
  ...
  test('删除叶子节点步骤包含 delete 类型', () => {
    var root = buildTree([50, 30, 70]);
    var result = deleteNode(root, 30);
    var deleteStep = result.steps.find(function(s) { return s.type === 'delete'; });
  ...
  test('删除双子节点步骤包含 replace 类型', () => {
    var root = buildTree([50, 30, 70]);
    var result = deleteNode(root, 50);
    var replaceStep = result.steps.find(function(s) { return s.type === 'replace'; });
...
describe('traverseTree', () => {
  ...
  test('前序遍历顺序正确', () => {
    var steps = traverseTree(root, 'pre');
  ...
  test('中序遍历顺序正确', () => {
    var steps = traverseTree(root, 'in');
    var visited = steps.filter(function(s) { return s.type === 'visit'; })
      .map(function(s) { return s.value; });
  ...
  test('后序遍历顺序正确', () => {
    var steps = traverseTree(root, 'post');
    var visited = steps.filter(function(s) { return s.type === 'visit'; })
      .map(function(s) { return s.value; });
  ...
  test('层序遍历顺序正确', () => {
    var steps = traverseTree(root, 'level');
    var visited = steps.filter(function(s) { return s.type === 'visit'; })
      .map(function(s) { return s.value; });
  ...
  test('空树遍历返回 done 步骤', () => {
    var steps = traverseTree(null, 'in');
  ...
  test('层序遍历描述包含层数信息', () => {
    var steps = traverseTree(root, 'level');
    var visitSteps = steps.filter(function(s) { return s.type === 'visit'; });
...
describe('treeHeight', () => {
  ...
  test('不平衡树高度正确', () => {
    var root = buildTree([10, 20, 30, 40, 50]);
...
describe('layoutTree', () => {
  test('空树返回空布局', () => {
    var layout = layoutTree(null);
    expect(layout.nodes).toEqual([]);
    expect(layout.edges).toEqual([]);
  });

  test('单节点树有 1 个节点 0 条边', () => {
    var layout = layoutTree(buildTree([50]));
    expect(layout.nodes.length).toBe(1);
    expect(layout.edges.length).toBe(0);
  });

  test('三个节点有 2 条边', () => {
    var layout = layoutTree(buildTree([50, 30, 70]));
  ...
  test('根节点 y 坐标为 0', () => {
    var layout = layoutTree(buildTree([50, 30, 70]));
    var rootNode = layout.nodes.find(function(n) { return n.value === 50; });
