 ...
 *
 * 所有工具的配置对象集中在此文件中定义。
 * tool-registry.js 从此文件导入数据并提供查询函数。
 */
...
const TOOLS = [
  // ── 计算机网络 ──
  {
    ...
    available: true,
    featured: true,
    tagline: '网络号、广播地址、可用主机范围等子网计算',
    taglineDetail: 'IP 地址与掩码输入后自动计算网络号、广播地址、可用主机范围等子网信息，32 位二进制与十进制对照展示',
    tags: ['#可视化', '#交互式'],
    difficulty: 'medium',
    hasSlowStartup: true,
    intro: {
  ...
  },
  {
    ...
    available: true,
    featured: true,
    tagline: '三次握手、四次挥手、丢包重传等动画演示',
    taglineDetail: '三次握手建连、四次挥手断连、丢包重传等 TCP 核心流程动画演示，SEQ/ACK 号与连接状态逐帧标注',
    tags: ['#可视化', '#交互式', '#面试必考'],
    difficulty: 'medium',
    intro: {
  ...
  },
  {
    ...
    available: true,
    featured: false,
    tagline: '初次握手、PSK 恢复、中间人警告等 TLS 1.3 握手演示',
    taglineDetail: '初次握手、PSK 恢复、中间人警告等三种 TLS 1.3 握手场景演示，报文交互动画、payload 字段级拆解、ECDHE 到 HKDF 密钥派生全链路展示',
    tags: ['#可视化', '#交互式', '#计算机网络'],
    difficulty: 'medium',
    intro: {
  ...
  },
  {
    ...
    available: true,
    featured: false,
    tagline: '递归查询、缓存命中、CNAME 链等 DNS 解析演示',
    taglineDetail: '域名输入后展示从根服务器到权威服务器的递归解析过程，缓存命中、CNAME 链等场景标注',
    tags: ['#可视化', '#交互式'],
    difficulty: 'medium',
    intro: {
  ...
  },
  {
    ...
    available: true,
    featured: false,
    tagline: '请求/响应报文、状态码、头部字段等 HTTP 解析',
    taglineDetail: 'HTTP 报文粘贴或预置例子选择后自动识别请求/响应类型，逐行拆分为请求行、状态行、头部字段及 inline 说明，另附 16 个状态码速查卡',
    tags: ['#可视化', '#交互式', '#计算机网络'],
    difficulty: 'easy',
    intro: {
  ...
  },
  {
    ...
    available: true,
    featured: false,
    tagline: '报文分片、MTU、偏移量计算等 IP 分片演示',
    taglineDetail: '报文大小和 MTU 滑块调节后自动计算分片列表（ID/MF/偏移量），偏移量运算过程展开查看，从最后一片向前的重组动画演示',
    tags: ['#可视化', '#交互式'],
    difficulty: 'medium',
    intro: {
  ...
  },
  {
    ...
    available: true,
    featured: false,
    tagline: 'SNAT、DNAT、端口映射等 NAT 模拟',
    taglineDetail: '单机上网、多机共享公网 IP、端口映射等三种 NAT 场景模拟，报文在 LAN/NAT/WAN 区域间动画展示，NAT 映射表实时刷新',
    tags: ['#可视化', '#交互式', '#计算机网络'],
    difficulty: 'medium',
    hasSlowStartup: true,
    intro: {
  ...
  },
  {
    ...
    available: true,
    featured: true,
    tagline: 'HTTPS、反向代理、HTTP 跳转等 Nginx 配置生成',
    taglineDetail: '域名、端口、证书路径等参数填写后自动生成格式化的 server block，支持 HTTPS、反向代理、HTTP 跳转 HTTPS 等常用场景配置',
    tags: ['#实用工具'],
    difficulty: 'easy',
    intro: {
  ...
  // ── 操作系统 ──
  {
    ...
    available: true,
    featured: false,
    tagline: 'FCFS、SJF、RR 等调度算法可视化',
    taglineDetail: 'FCFS、SJF、RR、MFQ 等四种调度算法切换与甘特图逐步回放，周转时间、等待时间、CPU 利用率、吞吐量等指标与 FCFS 基准对比展示',
    tags: ['#可视化', '#交互式', '#操作系统'],
    difficulty: 'medium',
    intro: {
  ...
  },
  {
    ...
    available: true,
    featured: false,
    tagline: '逻辑地址、页表、缺页中断、LRU/FIFO 等内存分页演示',
    taglineDetail: '页大小和帧数设定后输入逻辑地址序列，地址拆解、页表查询、缺页置换等过程动画演示，缺页率实时统计，LRU 与 FIFO 算法对比展示',
    tags: ['#可视化', '#交互式', '#操作系统'],
    difficulty: 'medium',
    order: 2
  },
  {
    ...
    available: true,
    featured: false,
    tagline: '资源分配图、银行家算法等死锁检测',
    taglineDetail: '进程和资源节点添加与分配边、请求边拖拽绘制后一键死锁检测，死锁进程与环路高亮标注。银行家算法模式中矩阵输入后 Need 自动计算与安全性判断',
    tags: ['#可视化', '#交互式', '#操作系统'],
    difficulty: 'medium',
    hasSlowStartup: true,
    intro: {
  ...
  },
  {
    ...
    available: true,
    featured: false,
    tagline: 'SCAN、C-SCAN、LOOK、C-LOOK 等磁盘调度演示',
    taglineDetail: 'SCAN、C-SCAN、LOOK、C-LOOK 等四种磁盘调度算法切换演示，磁头移动路径可视化，总寻道长度与平均寻道长度实时计算与多算法对比',
    tags: ['#可视化', '#交互式', '#操作系统'],
    difficulty: 'medium',
    order: 4
  },
  {
    ...
    available: true,
    featured: false,
    tagline: '生产者消费者、信号量、P/V 操作等同步互斥演示',
    taglineDetail: 'full/empty/mutex 三个信号量数值实时更新，环形缓冲区逐格展示生产与消费过程，P/V 操作日志记录、自动播放与步进模式',
    tags: ['#可视化', '#交互式', '#操作系统'],
    difficulty: 'medium',
    intro: {
  ...
  // ── 密码学 ──
  {
    ...
    available: true,
    featured: false,
    tagline: '密钥生成、加密解密、数论运算等 RSA 演算',
    taglineDetail: '素数选择、密钥生成、加密解密等三阶段 RSA 流程演示，欧拉函数计算、扩展欧几里得模逆求解、快速幂运算分解等中间步骤展开',
    tags: ['#交互式', '#进阶'],
    difficulty: 'advanced',
    intro: {
  ...
  },
  {
    ...
    available: true,
    featured: false,
    tagline: 'SubBytes、ShiftRows、MixColumns、AddRoundKey 等 AES 加密演示',
    taglineDetail: '16 字节明文与密钥输入后展示 SubBytes、ShiftRows、MixColumns、AddRoundKey 等每轮四步 AES 加密过程，密钥扩展 W[0] 至 W[43] 共 44 个字完整展示',
    tags: ['#可视化', '#进阶'],
    difficulty: 'advanced',
    intro: {
  ...
  },
  {
    ...
    available: true,
    featured: false,
    tagline: 'Diffie-Hellman 密钥交换、MITM 攻击等演示',
    taglineDetail: '素数与本原根设定后 Alice 和 Bob 双方的私钥生成、公钥交换、共享密钥计算等 Diffie-Hellman 流程演示，MITM 攻击模式中 Eve 的拦截篡改过程展示',
    tags: ['#可视化', '#进阶'],
    difficulty: 'advanced',
    intro: {
  ...
  },
  {
    ...
    available: true,
    featured: false,
    tagline: '消息填充、64 轮压缩、雪崩效应等 SHA-256 演示',
    taglineDetail: '文本输入后 64 轮压缩函数完整运算展示，消息填充、轮常数映射、单 bit 修改对摘要的雪崩效应等过程演示',
    tags: ['#可视化', '#进阶'],
    difficulty: 'advanced',
    intro: {
  ...
  },
  {
    ...
    available: true,
    featured: false,
    tagline: '凯撒、维吉尼亚、栅栏等古典密码加解密',
    taglineDetail: '凯撒、维吉尼亚、栅栏等三种古典密码切换演示，凯撒 25 种移位结果列举、维吉尼亚逐字母密钥循环展示、栅栏点阵结构可视化，附字母频率柱状图与英文标准频率对比',
    tags: ['#交互式', '#实用工具'],
    difficulty: 'easy',
    intro: {
  ...
  // ── 算法 & 数据结构 ──
  {
    ...
    available: true,
    featured: true,
    tagline: '选择、冒泡、快排等排序算法演示',
    taglineDetail: '选择排序、冒泡排序、快速排序等三种排序算法动画演示，比较与交换操作步骤展示，播放速度可调',
    tags: ['#可视化', '#交互式'],
    difficulty: 'easy',
    intro: {
  ...
  },
  {
    ...
    available: true,
    featured: true,
    tagline: 'BST、栈、队列、哈希表、图等数据结构演示',
    taglineDetail: 'BST、栈、队列、哈希表、图等五种数据结构交互操作演示，插入与删除操作中内部状态实时更新，BFS 与 DFS 搜索过程对比展示',
    tags: ['#可视化', '#交互式', '#面试必考'],
    difficulty: 'medium',
    intro: {
  ...
  },
  {
    ...
    available: true,
    featured: false,
    tagline: '节点分裂、范围查询、阶数可调等 B+ 树演示',
    taglineDetail: '阶数 4 至 32 可调，B+ 树节点分裂与合并过程展示，单 key 查询与范围查询路径高亮演示',
    tags: ['#可视化', '#进阶'],
    difficulty: 'advanced',
    intro: {
  ...
  // ── 编译原理 ──
  {
    ...
    available: true,
    featured: false,
    tagline: '正则转 NFA、子集构造 DFA、状态图等词法分析演示',
    taglineDetail: '*、|、.、()、?、+ 等常用正则语法支持，NFA 状态图生成、子集构造法 DFA 转换、转移表与状态图输出，DFA 模拟运行展示匹配结果',
    tags: ['#可视化', '#交互式', '#编译原理'],
    difficulty: 'advanced',
    intro: {
  ...
  },
  {
    ...
    available: true,
    featured: false,
    tagline: 'FIRST 集、FOLLOW 集、预测分析表等 LL(1) 分析',
    taglineDetail: '上下文无关文法输入（→ 分隔、| 多选、ε 空串）后 FIRST 集与 FOLLOW 集自动计算，LL(1) 预测分析表生成与冲突高亮，字符串分析过程中栈、输入缓冲、输出序列同步展示',
    tags: ['#可视化', '#交互式', '#编译原理'],
    difficulty: 'advanced',
    hasSlowStartup: true,
    intro: {
  ...
  },
  {
    ...
    available: true,
    featured: false,
    tagline: 'Token 流、符号表、最长匹配等词法分析演示',
    taglineDetail: '类 C 代码输入或预置例子选择后自动进行词法分析，源码 Token 按类型着色展示、Token 卡片详情查看、符号表汇总标识符出现位置，支持步进模式与全量运行，最长匹配原则与关键字优先级处理展示',
    tags: ['#可视化', '#编译原理'],
    difficulty: 'medium',
    intro: {
  ...
  },
  {
    ...
    available: true,
    featured: false,
    tagline: '词法分析、LL(1) 解析、AST 构建等编译演示',
    taglineDetail: '表达式输入后经过词法分析、LL(1) 解析、AST 树构建到 SDT 求值等三阶段编译演示，Token 流逐步高亮、递归下降产生式记录、AST 树节点颜色编码与步进求值展示',
    tags: ['#可视化', '#交互式', '#编译原理'],
    difficulty: 'medium',
    intro: {
