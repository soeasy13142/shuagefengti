...
- **已上线 11 个工具**（`utils/tool-registry.js`）：
  ...
  - Nginx 生成器 `nginx-gen` · 死锁模拟器 `deadlock`
- **首页重设计** — 工具箱优先布局 · 分类标签栏 · 2 列网格 · 简洁/详细双模式
- **设计系统** — Claude Design 暖奶油画布 · intro-modal 向导式分步组件 · 底部固定底栏
- **工程规范** — var→const/let 清零 · 纯函数 + 不可变 · TDD（44 suites / 705 tests 全绿）
...
| | |
|---|---|
| **Working** | 全部 11 个工具 + 刷题主链路。`npm test` 705 tests / 44 suites 全绿 |
| **Broken** | 无已知问题 |
| **Blocked** | 无 |
...
按用户需求从 tool-registry 中选择下一工具上线（13 个待开发）。
