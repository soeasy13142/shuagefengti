...
`刷个冯题` 是一个微信小程序学习工具箱。当前已完成核心的 **Markdown 试题导入 + 本地刷题 + 练习/考试模式 + 答题记录 + 错题本**，并额外上线了 **排序可视化**、**学习驾驶舱** 和 **子网计算器** 模块。项目采用微信小程序原生框架，纯前端、本地存储、无后端。
...
10. 当前测试全部通过：**7 个测试套件，96 个测试用例全部通过**。
...
1. 首页新增入口卡片：`子网计算器`；
2. 新增页面：`pages/subnet-calc/subnet-calc`；
3. 输入区域：
   - IP 地址输入框（四段十进制，自动校验 0-255）；
   - CIDR 前缀长度滑块或输入框（/1 ~ /30）；
   - 快捷按钮：/8、/16、/24、/25、/26、/27、/28、/29、/30；
4. 实时计算结果：
   - 子网掩码（四段十进制 + 二进制）；
   - 网络地址；
   - 广播地址；
   ...
   - 网络地址的二进制表示；
5. 二进制位可视化（核心炫技点）：
   - 32 个 bit 格子，每 8 个一组用空格分隔；
   - 网络位用一种颜色高亮（如珊瑚色）；
   - 主机位用另一种颜色（如浅蓝）；
   - 输入 IP 时实时显示其 32 位二进制；
   - 子网掩码的 1 和 0 对应颜色显示；
   - 网络地址 = IP AND Mask 的位运算过程动画；
6. 子网信息卡片：
   - 子网类型判断（A/B/C 类，私有地址判断）；
   - 地址用途说明（如 127.x.x.x 是环回地址）；
